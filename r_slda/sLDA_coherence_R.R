...
# Set seed before splitting so the partition is reproducible
set.seed(2025)

# coherencetext_training.csv / coherencetext_test.csv were referenced by the
# original script but never existed in the data folder, so this script could
# not run. The split is made here instead, grouped on participant so that the
# same person cannot appear in both the training and prediction sets. The
# participant id is the part of the filename before the "@".
coherence <- read.csv(file.path(data_dir, "coherencetext.csv"))
coherence$uid <- sub("@.*$", "", coherence$file)

uids <- unique(coherence$uid)
test_uids <- sample(uids, size = max(1, round(0.3 * length(uids))))

df             <- coherence[!coherence$uid %in% test_uids, ]
for_prediction <- coherence[ coherence$uid %in% test_uids, ]

cat("participants:", length(uids),
    "| train rows:", nrow(df),
    "| test rows:", nrow(for_prediction), "\n")
